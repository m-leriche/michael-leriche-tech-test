...
export function Form() {
  ...
  const [formData, setFormData] = useState<FormData>({
	...
	const handleChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if ((name === 'firstName' || name === 'lastName') && value.length > 50) return;
...
return (
	...
	>
	  <h1 id="onboarding-heading" className="text-2xl font-medium text-center mb-6">Onboarding Form</h1>
	  ...
	  <button
		...
		{success && <div className='flex flex-col items-center' role="status" aria-live="polite">
			<p className="text-green-600 text-sm mt-4 text-center">{success}</p>
			<img src={beanImage} alt="Mr. Bean tells you nice job!" className="w-[100px] p-2" />
			</div>
		}
	</form>
  );
